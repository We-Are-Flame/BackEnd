...
public enum SortType {
    ...
    SortType(String field, Direction direction) {
        this.field = field;
    ...
    public static Sort getSort(String sort) {
        ...
        return Sort.by(new Sort.Order(sortType.direction, sortType.field));
    }

    public static OrderSpecifier<?>[] createPageableSort(Pageable pageable) {
        return pageable.getSort().stream()
                .flatMap(order -> SortType.fromString(order.getProperty()).toOrderSpecifiers()
                        .stream())
    ...
    private static SortType fromString(String key) {
